...
namespace FileUpload
{
    ...
    {
        //我们定义Buffer为1000
        public const int BufferCount = 1000;
        ...
        public void WriteToServer(string filePath, int startPositon, byte[] btArray)
        {
        ...
        public void UpLoadFileFromLocal(string localFilePath, string uploadFilePath, int startPostion, int totalCount)
        {
            ...
            var tempReadCount = 0;
            //定义一个缓冲区数组
            var bufferByteArray = new byte[BufferCount];
            //定义一个FileStream对象
            ...
            using (fileStream)
            {
                ...
                while (tempReadCount < totalCount)
                {
                    ...
                    if (tempBuffer + tempReadCount > totalCount)
                    {
                        ...
                        if (tempBuffer > 0)
                        {
                            var newTempBtArray = new byte[tempBuffer];
                            Array.Copy(bufferByteArray, 0, newTempBtArray, 0, tempBuffer);
                            //将缓冲区的数据上传至服务器
                            WriteToServer(uploadFilePath, writeStartPosition, newTempBtArray);
                        }

                    }
                    //如果缓冲区的数据量小于该段数据量，并且tempBuffer=设定BUFFER_COUNT时，通过
                    //while 循环每次读取一样的buffer值的数据写入服务器中，直到将该段数据全部处理完毕
                    else if (tempBuffer == BufferCount)
                    {
                        fileStream.Read(bufferByteArray, 0, tempBuffer);
                        WriteToServer(uploadFilePath, writeStartPosition, bufferByteArray);
                    }
        ...
        public void UpLoadFileFromLocal(FileStream fileStream, string uploadFilePath, int startPostion, int totalCount)
        {
            //if(!File.Exists(localFilePath)){return;}
            //每次临时读取数据数
            var tempReadCount = 0;
